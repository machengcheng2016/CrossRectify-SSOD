...
class SSD(nn.Module):
    ...
    def forward(self, x):
        ...
        loc = torch.cat([o.view(o.size(0), -1) for o in loc], 1)    # (batch_size, 34928=8732*4), which later be resized as (batch_size, 8732, 4)
        conf = torch.cat([o.view(o.size(0), -1) for o in conf], 1)  # (batch_size, 183372=8732*21), which later be resized as (batch_size, 8732, 21)
