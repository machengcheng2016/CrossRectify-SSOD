...
def train():
    ...
    for iteration in range(cfg['max_iter'] // (args.batch_size // 32)):
                ...
                if mask_val_fg.sum() > 0:
                    ...
                    if mask_pseudo.sum() > 0:
                        ...
                        if mask_correct.sum() > 0:
                            ...
                            print(conf_data_unsup_i_selected[mask_correct,:].shape[0], torch.sum(conf_data_unsup_i_selected[mask_correct,:].argmax(dim=-1) == label_unsup_i_selected[mask_correct]).item())
            if N > 0:
        ...
        if iteration != 0 and (iteration+1) % args.save_interval == 0:
            ...
            torch.save(ssd_net.state_dict(), os.path.join(args.save_folder, "{}+{}".format(args.sup_aug_type, args.unsup_aug_type), save_path, "ssd300_pseudo36_" + repr(iteration+1) + '.pth'))
