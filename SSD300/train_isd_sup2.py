...
from ssd_source2 import build_ssd
import os
import sys
import time
import torch
...
def train():
    ...
    for iteration in range(cfg['max_iter'] // (args.batch_size // 32)):
        ...
        if len(sup_image_index) < 2:
            continue

        images = images[sup_image_index]
        ...
        output, sources = net(images)
        loc, conf, priors = output
        
        output_shuffle, sources_shuffle = net(images_shuffle)
        ...
        if mask_left_right.sum() > 0 or mask_only_left.sum() > 0:
            out_mix, sources_mix = net(images_mix)
        ...
        if iteration % 200 == 0:
            ...
            print("iter {} || loss: {:.4f}, loss_c: {:.4f}, loss_l: {:.4f}, loss_c_unsup1: {:.4f}, loss_c_unsup: {:.4f}, loss_l_unsup: {:.4f}, lr: {:.4f}, super_len: {}, mask_only_left.sum() = {}, lam = {}\n".format(iteration, loss.item(), loss_c.item(), loss_l.item(), loss_c_unsup1.item(), loss_c_unsup.item(), loss_l_unsup.item(), float(optimizer.param_groups[0]['lr']), len(sup_image_index), mask_only_left.sum().item(), lam))
        ...
        if iteration != 0 and (iteration+1) % args.save_interval == 0:
            ...
            if args.lam < 0:
                torch.save(ssd_net.state_dict(), os.path.join(args.save_folder, "{}+{}".format(args.sup_aug_type, args.unsup_aug_type), 'ssd300_isd_sup2_' + repr(iteration+1) + '.pth'))
            else:
                torch.save(ssd_net.state_dict(), os.path.join(args.save_folder, "{}+{}".format(args.sup_aug_type, args.unsup_aug_type), "ssd300_isd_{}_sup2_".format(args.lam) + repr(iteration+1) + '.pth'))
    print('-------------------------------\n')
    print(loss.item())
    print('-------------------------------')
