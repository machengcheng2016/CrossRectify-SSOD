...
def match(targets, priors, variances=[0.1, 0.2], threshold=0.5):
...
def train():
    ...
    for iteration in range(cfg['max_iter'] // (args.batch_size // 32)):
        ...
        if len(unsup_image_index) > 0:
            ...
            losses_c_unsup_correct_all, losses_c_unsup_incorrect_all = None, None
            ...
            for i_unsup in range(len(unsup_image_index)):
                ...
                if mask_val_fg.sum() > 0:
                    ...
                    if mask_pseudo.sum() > 0:
                        ...
                        anchor_label_unsup_i_selected = anchor_label_unsup_i[mask_val_fg][mask_pseudo][indices_nms]
                        assert anchor_label_unsup_i_selected.shape == label_unsup_i_selected.shape
                        ...
                        N += conf_data_unsup_i_selected.shape[0]
                        n += torch.sum(conf_data_unsup_i_selected.argmax(dim=-1) == label_unsup_i_selected).item()
                        
                        losses_c_unsup = F.cross_entropy(conf_data_unsup_i_selected, conf_data_unsup_i_selected.argmax(dim=-1), size_average=False, reduce=False)
            
            if len(losses_c_unsup) > 0:
                loss_c_unsup = losses_c_unsup.sum(-1)
        ...
        if iteration != 0 and (iteration+1) % args.save_interval == 0:
            ...
            torch.save(ssd_net.state_dict(), os.path.join(args.save_folder, "{}+{}".format(args.sup_aug_type, args.unsup_aug_type), save_path, "ssd300_pseudo39_" + repr(iteration+1) + '.pth'))
