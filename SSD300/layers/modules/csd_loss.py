...
class CSDLoss_select(nn.Module):
    def __init__(self, use_gpu=True):
        super(CSDLoss_select, self).__init__()
        self.use_gpu = use_gpu

    def forward(self, args, conf, conf_flip, loc, loc_flip, conf_consistency_criterion, sup_image_index):
        conf = conf[sup_image_index,:,:]
        conf_flip = conf_flip[sup_image_index,:,:]
        loc = loc[sup_image_index,:,:]
        loc_flip = loc_flip[sup_image_index,:,:]
