...
def train():
    ...
    for iteration in range(cfg['max_iter'] // (args.batch_size // 32)):
        ...
        if iteration != 0 and (iteration+1) % args.save_interval == 0:
            ...
            torch.save(ssd_net.state_dict(), os.path.join(args.save_folder, "{}+{}".format(args.sup_aug_type, args.unsup_aug_type), save_path, "ssd300_pseudo151_student_betterInitial_A_{}_{}_".format(args.ema_rate, args.ema_step) + repr(iteration+1) + '.pth'))
            torch.save(ssd_net2.state_dict(), os.path.join(args.save_folder, "{}+{}".format(args.sup_aug_type, args.unsup_aug_type), save_path, "ssd300_pseudo151_student_betterInitial_B_{}_{}_".format(args.ema_rate, args.ema_step) + repr(iteration+1) + '.pth'))
            torch.save(ssd_net_teacher.state_dict(), os.path.join(args.save_folder, "{}+{}".format(args.sup_aug_type, args.unsup_aug_type), save_path, "ssd300_pseudo151_teacher_betterInitial_A_{}_{}_".format(args.ema_rate, args.ema_step) + repr(iteration+1) + '.pth'))
            torch.save(ssd_net2_teacher.state_dict(), os.path.join(args.save_folder, "{}+{}".format(args.sup_aug_type, args.unsup_aug_type), save_path, "ssd300_pseudo151_teacher_betterInitial_B_{}_{}_".format(args.ema_rate, args.ema_step) + repr(iteration+1) + '.pth'))
