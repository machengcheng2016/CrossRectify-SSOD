...
class VOCDetection_con(data.Dataset):
    ...
    def __init__(self, root,
                 image_sets=[('2007', 'trainval'), ('2012', 'trainval')],
                 img_transform_sup=None, img_transform_unsup=None, target_transform=VOCAnnotationTransform_con(),
                 dataset_name='VOC0712', white_box=False, random_flag=True):
        self.root = root
        self.coco_root = ''
        self.image_set = image_sets
        self.img_transform_sup = img_transform_sup
        self.img_transform_unsup = img_transform_unsup
    ...
    def pull_item(self, index):
        ...
        if semi and self.img_transform_sup is not None:
            img, boxes, labels = self.img_transform_sup(img, target[:, :4], target[:, 4])
            img = img[:, :, (2, 1, 0)]  # to rgb
            target = np.hstack((boxes, np.expand_dims(labels, axis=1)))

        if (not semi) and self.img_transform_unsup is not None:
            img, boxes, labels = self.img_transform_unsup(img, target[:, :4], target[:, 4])
            img = img[:, :, (2, 1, 0)]  # to rgb
            target = np.hstack((boxes, np.expand_dims(labels, axis=1)))
