    ...
    for iteration in range(cfg['max_iter'] // (args.batch_size // 32)):
        ...
        if len(unsup_image_index) > 0:
            ...
            for i_unsup in range(len(unsup_image_index)):
                ...
                mask_val_fg = (each_val > thresholding(iteration)).data
        ...
        if iteration != 0 and (iteration+1) % args.save_interval == 0:
            ...
            torch.save(ssd_net.state_dict(), os.path.join(args.save_folder, "{}+{}".format(args.sup_aug_type, args.unsup_aug_type), save_path, "ssd300_pseudo39_0.5_to_0.8_" + repr(iteration+1) + '.pth'))
...
def thresholding(iteration):
    if iteration < 12000:
        ...
        threshold = 0.5 + (0.8-0.5) / (120000-12000) * (iteration-12000)
    return threshold
