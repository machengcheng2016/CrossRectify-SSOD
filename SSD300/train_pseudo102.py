...
def train():
    ...
    for iteration in range(cfg['max_iter'] // (args.batch_size // 32)):
            ...
            for i_unsup in range(len(unsup_image_index)):
                ...
                if mask_val_fg.sum() > 0:
                    ...
                    if mask_pseudo.sum() > 0:
                        ...
                        label_unsup_i_selected2 = label_unsup_i_selected.detach().clone()                        
                        mask_incorrect = (label_unsup_i_selected2 != conf_data_unsup_i_selected.argmax(dim=-1)).data
                        if mask_incorrect.sum() > 0:
                            label_unsup_i_selected2[mask_incorrect] += torch.randint(1, 20, (mask_incorrect.sum().item(),))
                            label_unsup_i_selected2[label_unsup_i_selected2 >= 21] -= 21
                        
                        loss_c_unsup += F.cross_entropy(conf_data_unsup_i_selected, label_unsup_i_selected2, size_average=False, reduce=False).sum(-1)
        ...
        if iteration != 0 and (iteration+1) % args.save_interval == 0:
            ...
            torch.save(ssd_net.state_dict(), os.path.join(args.save_folder, "{}+{}".format(args.sup_aug_type, args.unsup_aug_type), save_path, "ssd300_pseudo102_{}.pth".format(iteration+1)))
