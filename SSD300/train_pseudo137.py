...
def train():
    ...
    for iteration in range(cfg['max_iter'] // (args.batch_size // 32)):
            ...
            for i_unsup in range(len(unsup_image_index)):
                ...
                if mask_val_fg.sum() > 0:
                    ...
                    if mask_pseudo.sum() > 0:
                        ...
                        n1 += torch.sum(conf_data_unsup_i_selected_teacher.argmax(dim=-1) == label_unsup_i_selected).item()
            ...
            for i_unsup in range(len(unsup_image_index2)):
                ...
                if mask_val_fg2.sum() > 0:
                    ...
                    if mask_pseudo2.sum() > 0:
                        ...
                        n2 += torch.sum(conf_data_unsup_i_selected2_teacher.argmax(dim=-1) == label_unsup_i_selected2).item()
        ...
        if iteration != 0 and (iteration+1) % args.save_interval == 0:
            ...
            torch.save(ssd_net.state_dict(), os.path.join(args.save_folder, "{}+{}".format(args.sup_aug_type, args.unsup_aug_type), save_path, "ssd300_pseudo137_A_{}_{}_".format(args.ema_rate, args.ema_step) + repr(iteration+1) + '.pth'))
            torch.save(ssd_net2.state_dict(), os.path.join(args.save_folder, "{}+{}".format(args.sup_aug_type, args.unsup_aug_type), save_path, "ssd300_pseudo137_B_{}_{}_".format(args.ema_rate, args.ema_step) + repr(iteration+1) + '.pth'))
