...
def main(args):
    ...
    if cfg.SEMISUPNET.Trainer == "ubteacher":
        ...
        ensem_ts_model.modelTeacher.train(False)
        import torch
        for idx, dataset_name in enumerate(cfg.DATASETS.TEST):
            ...
            for idx, inputs in enumerate(data_loader):
                with torch.no_grad():
                    outputs = ensem_ts_model.modelTeacher(inputs)
                ...
                for iii in range(len(pred_classes)):
                    ...
                    res += [tmp_dir]
                if (idx+1) % 500 == 0:
                    print("Test Dataset: {}, {} images are processed by modelTeacher in {}".format(dataset_name, idx+1, (cfg.MODEL.WEIGHTS).split('/')[-1]))
    ...
    with open(os.path.join("output", json_name+"-teacher-output.json"), 'w') as fid:
        json.dump(res, fid)
