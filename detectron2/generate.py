...
from ubteacher.modeling.meta_arch.rcnn import TwoStagePseudoLabDoubleGeneralizedRCNN
...
def main(args):
    ...
    if args.eval_only:
        if cfg.SEMISUPNET.Trainer == "ubteacher":
            ...
            model_ema = Trainer.build_model(cfg)
            ensem_ts_model = EnsembleTSModel(model, model_ema)

            DetectionCheckpointer(
                ensem_ts_model, save_dir=cfg.OUTPUT_DIR
            ).resume_or_load(cfg.MODEL.WEIGHTS, resume=args.resume)
            Trainer.generate(cfg, ensem_ts_model.modelStudent)
            #Trainer.generate(cfg, ensem_ts_model.modelTeacher)
