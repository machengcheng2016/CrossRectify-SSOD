...
class DetectionTSCheckpointer(DetectionCheckpointer):
    def _load_model(self, checkpoint):
        if checkpoint.get("__author__", None) == "Caffe2":
            ...
            if checkpoint.get("matching_heuristics", False):
                print("/dockerdata/brunoma/mine3-unbiased-teacher-VOC07-sup-VOC12-unsup-cotraining2-0.7-discarding_empty_images/ubteacher/checkpoint/detection_checkp/ 49")
                self._convert_ndarray_to_tensor(checkpoint["model"])
                # convert weights by name-matching heuristics
                checkpoint["model"] = align_and_update_state_dicts(
